...
#        print('Average number of words per sentence: ', np.mean([len(s.split(" ")) for s in self.test_A.tweet]))
        ...
        for i in range(0, len(self.test_A)):
            # Tokenize tweets
            test_words = word_tokenize(self.test_A.iloc[i][2])
    ...
    def readTrain(self):
        ...
        self.train_A = pd.read_csv(train_file_A, sep='\t', header=None, names=data_fields,
                                   quoting=3)  # quoting=3 tells Python to ignore doubled quotes, header=None defines that the  first line of the file is not the names of the columns

        self.test_A = pd.read_csv(test_file_A, sep='\t', header=None, names=data_fields,
                                   quoting=3)  # quoting=3 tells Python to ignore doubled quotes, header=None defines that the  first line of the file is not the names of the columns
